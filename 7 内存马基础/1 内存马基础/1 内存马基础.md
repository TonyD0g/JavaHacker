...
- EndPoint（ProtocolHandler的组成之⼀）

- Processor（ProtocolHandler的组成之⼀）
...
```md
主要是⽤来控制Tomcat所能接收的最⼤数量连接，如果超过了此连接，那么Tomcat就会将
此连接线程阻塞等待，等⾥⾯有其他连接释放了再消费此连接
```

​	Acceptor：

```md
负责接收新的连接，然后返回⼀个Channel对象给Poller
```
...
调⽤events()⽅法，查看队列中是否有Pollerevent事件，如果有就将其取出，然后把⾥⾯的Channel取出来注册到该Selector中，然后不断轮询所有注册过的Channel查看是否有事件发⽣。
```
...
```md
可以看成是⼀个被封装的任务类
当有事件发⽣时，则调⽤SocketProcessor交给Executor执⾏
```
...
```md
Tomcat⾃⼰扩展的线程池，⽤来执⾏任务类.
在Tomcat中Executor由Service维护，因此同⼀个Service中的组件可以共享⼀个线程池。如果没有定义任何线程池，相关组件( 如Endpoint)会⾃动创建线程池，此时，线程池不再共享。
```


EndPoint内部图示:![](Pic/Endpoint.jpg)
...
### [pipeline机制](https://www.cnblogs.com/coldridgeValley/p/5816414.html)
